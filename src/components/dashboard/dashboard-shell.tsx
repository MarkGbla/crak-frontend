...
import { Bell, ChevronDown, Gift, Home, KeyRound, LifeBuoy, Menu, Settings, UsersRound, WalletCards, X } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#f6f8f5] text-[#17221b]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[244px] flex-col border-r border-[#dde4de] bg-white px-4 pb-5 pt-6 lg:flex">
        ...
        <button data-tour="workspace" className="mt-7 flex items-center gap-3 rounded-xl border border-[#dfe6e0] p-3 text-left" type="button">
          <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#e6f5eb] text-xs font-extrabold text-[#087a4f]">FC</span>
          <span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold">Freetown Coffee Co.</span><span className="mt-0.5 block text-[10px] text-[#7b877f]">Active workspace</span></span>
          <ChevronDown size={14} className="text-[#849087]" />
        </button>
        ...
        <button className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-[#5e6a62] hover:bg-[#f4f7f4]" type="button"><LifeBuoy size={18} /> Help & support</button>
      </aside>
      ...
      <div className="lg:pl-[244px]">
        <header className="sticky top-0 z-20 flex h-[68px] items-center justify-between border-b border-[#dde4de] bg-white/92 px-4 backdrop-blur-xl sm:px-7 lg:px-9">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <ProductTour />
            <button className="relative grid size-10 place-items-center rounded-lg border border-[#d9e1db] bg-white text-[#59675e]" aria-label="Notifications"><Bell size={17} /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-[#df5e42]" /></button>
            <button className="grid size-10 place-items-center rounded-full bg-[#173b2a] text-xs font-bold text-white" aria-label="Open account menu">MK</button>
          </div>
        </header>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-[#102219]/45 backdrop-blur-sm lg:hidden" onMouseDown={() => setMobileOpen(false)}>
          <aside className="flex h-full w-[284px] flex-col bg-white px-4 pb-5 pt-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="mt-6 flex items-center gap-3 rounded-xl border border-[#dfe6e0] p-3"><span className="grid size-9 place-items-center rounded-lg bg-[#e6f5eb] text-xs font-extrabold text-[#087a4f]">FC</span><div><p className="text-xs font-bold">Freetown Coffee Co.</p><p className="mt-0.5 text-[10px] text-[#7b877f]">Active workspace</p></div></div>
            ...
            <button className="flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-semibold text-[#5e6a62]"><LifeBuoy size={18} />Help & support</button>
          </aside>
        </div>
      )}
